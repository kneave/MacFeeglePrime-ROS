...
def callback(data):    
    ...
    setservos(data.data[0], data.data[1], data.data[2], data.data[3], data.data[4], data.data[5], data.data[6])
...
def listener():
    ...
    rospy.Subscriber('arm_controller', Int16MultiArray, callback)

    # spin() simply keeps python from exiting until this node is stopped
    rospy.spin()

def setservos(flappy, rotator, upperarm, elbow, wrist_rotate, wrist_pivot, hand):
